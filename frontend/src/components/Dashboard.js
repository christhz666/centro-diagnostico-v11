...
const StatCard = ({ title, value, subtext, icon, colorClass }) => {
    return (
        <div className="bg-[#1d2027]/70 backdrop-blur-[24px] p-6 rounded-xl shadow-[0_0_40px_-15px_rgba(74,253,239,0.15)] group hover:bg-[#1d2027] transition-all border border-white/5 flex flex-col justify-between h-48">
            ...
            <div>
                <p className="font-label text-[10px] text-[#bacac7] uppercase tracking-[0.15em] mb-1">{title}</p>
                <h3 className="font-headline text-3xl font-bold text-white">{value}</h3>
                <p className="text-[10px] text-[#bacac7] mt-2">{subtext}</p>
            </div>
        </div>
    );
};
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6 relative z-10">
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-2 text-[#bacac7]">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-lg border border-[#3b4a48]/50 hover:bg-[#272a31]/50 transition-colors text-[#e0e2ec] active:scale-95 ${loading ? 'opacity-50' : ''}`}
                >
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12 relative z-10">
                ...
                <div className="lg:col-span-2 bg-[#1d2027]/70 backdrop-blur-[24px] rounded-xl p-8 border border-white/5">
                    <div className="flex justify-between items-center mb-10">
                        <div>
                            <h4 className="font-headline text-lg font-bold text-white uppercase tracking-tight">Pacientes por Día</h4>
                            <p className="font-label text-[10px] text-[#bacac7] uppercase tracking-widest">Últimos 30 días</p>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-[#4afdef]"></span>
                                <span className="text-[10px] font-label text-[#bacac7] uppercase tracking-widest">Total</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-[#10b981]"></span>
                                <span className="text-[10px] font-label text-[#bacac7] uppercase tracking-widest">Completadas</span>
                            </div>
                        </div>
                    </div>
                    {citasGrafica.length > 0 ? (
                        ...
                        <div className="h-64 flex flex-col items-center justify-center text-[#bacac7]/50">
                ...
                <div className="bg-[#1d2027]/70 backdrop-blur-[24px] rounded-xl p-8 border border-white/5 flex flex-col">
                    <div className="w-full text-left mb-8">
                        <h4 className="font-headline text-lg font-bold text-white uppercase tracking-tight">Estudios</h4>
                        <p className="font-label text-[10px] text-[#bacac7] uppercase tracking-widest">Distribución por especialidad</p>
                    </div>
                                ...
                                {topEstudios.slice(0,3).map((est, i) => (
                                    <div key={i} className="flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-label text-[10px] text-[#e0e2ec] uppercase tracking-wider truncate max-w-[120px]">{est.nombre}</span>
                                        </div>
                                        <span className="font-headline text-xs text-white">{est.cantidad}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ) : (
                        <div className="flex-1 flex flex-col items-center justify-center text-[#bacac7]/50">
            ...
            {citasGrafica.length > 0 && (
                <section className="bg-[#1d2027]/70 backdrop-blur-[24px] rounded-xl p-8 border border-white/5 mb-12 relative z-10">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            <h4 className="font-headline text-lg font-bold text-white uppercase tracking-tight">Actividad de Citas</h4>
                            <p className="font-label text-[10px] text-[#bacac7] uppercase tracking-widest">Total vs Completadas — últimos 30 días</p>
                        </div>
                        <span className="material-symbols-outlined text-[#bacac7]" style={{ fontVariationSettings: "'FILL' 0, 'wght' 300, 'GRAD' 0, 'opsz' 24" }}>bar_chart</span>
                    </div>
                    <div className="h-64">
                         <ResponsiveContainer width="100%" height="100%">
            ...
            <section className="bg-[#1d2027]/70 backdrop-blur-[24px] rounded-xl overflow-hidden border border-white/5 relative z-10">
                <div className="px-8 py-6 flex justify-between items-center bg-[#191b23]/30">
                    <h4 className="font-headline text-lg font-bold text-white uppercase tracking-tight">Pacientes de Hoy</h4>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-[#10131a]/50 border-none">
                            <tr>
                                <th className="px-8 py-4 font-label text-[10px] text-[#bacac7] uppercase tracking-[0.2em] font-medium">Paciente</th>
                                <th className="px-8 py-4 font-label text-[10px] text-[#bacac7] uppercase tracking-[0.2em] font-medium">Estudio</th>
                                <th className="px-8 py-4 font-label text-[10px] text-[#bacac7] uppercase tracking-[0.2em] font-medium">Estado</th>
                                <th className="px-8 py-4 font-label text-[10px] text-[#bacac7] uppercase tracking-[0.2em] font-medium">Hora</th>
                                <th className="px-8 py-4 font-label text-[10px] text-[#bacac7] uppercase tracking-[0.2em] font-medium text-right">Acción</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y-0">
                                ...
                                <tr key={i} className="group hover:bg-white/5 transition-colors">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-9 h-9 rounded-full bg-[#32353c] flex items-center justify-center font-headline text-sm font-bold text-[#4afdef]">
                                                {(cita.paciente?.nombre || 'P')[0]}
                                            </div>
                                            <div>
                                                <p className="font-headline text-sm text-white font-medium">{cita.paciente?.nombre} {cita.paciente?.apellido}</p>
                                                <p className="font-label text-[10px] text-[#bacac7] uppercase tracking-widest">ID: {cita.paciente_id}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                                        <span className="bg-[#32353c] text-[#e0e2ec] px-3 py-1 rounded text-[10px] font-label uppercase tracking-widest">
                                    ...
                                    <td className="px-8 py-5 font-headline text-sm text-[#bacac7]">
                                        {cita.horaInicio || '--:--'}
                                    </td>
                                    <td className="px-8 py-5 text-right">
                                        <button
                                            onClick={() => window.location.href = `/resultados?paciente=${cita.paciente_id}`}
                                            className="p-2 hover:bg-[#32353c] rounded transition-colors text-[#bacac7] hover:text-white"
                                        >
                            ...
                            {citasHoy.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center text-[#bacac7]">No hay citas para hoy</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                </div>
                <div className="px-8 py-4 bg-[#191b23]/30 flex justify-center border-t border-white/5">
                    <button

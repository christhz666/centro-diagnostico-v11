    ...
    <div className="relative font-body text-[#e1e2eb] min-h-[calc(100vh-4rem)] p-8 overflow-hidden">
      ...
      <div className="flex flex-col gap-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <span className="text-[#bacac7] font-label text-[10px] tracking-[0.3em] uppercase block mb-1">Clinical Directory</span>
            ...
            <p className="text-[#bacac7] mt-1 text-sm">Manage current admissions and diagnostic pipelines</p>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            {/* Search Input directly in header for quick access without TopBar */}
            <div className="relative w-full md:w-80 group">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#bacac7] group-focus-within:text-[#00e0d3] transition-colors" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>search</span>
              <input 
                className="w-full bg-[#32353c] border-none rounded-lg py-2.5 pl-10 pr-4 text-[#e1e2eb] focus:ring-1 focus:ring-[#4afdef]/30 transition-all placeholder:text-[#bacac7]/50 text-sm" 
        ...
        <div className="bg-[rgba(29,32,38,0.7)] backdrop-blur-[24px] rounded-xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] flex h-[calc(100vh-280px)] border border-white/5 overflow-hidden">
          
          {/* Main List (Left/Center) */}
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            {!resultados && termino.length < 2 && (
              <div className="h-full flex flex-col items-center justify-center text-[#bacac7] opacity-60">
            ...
            {resultados && resultCount === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-[#bacac7]">
            ...
            {resultados && resultCount > 0 && (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="text-[10px] uppercase tracking-widest text-[#bacac7] border-b border-white/10">
                ...
                <tbody className="divide-y divide-white/5">
                  ...
                  {resultados.pacientes.map(p => (
                    <tr key={`p-${p.id}`} onClick={() => verHistorial(p.id)} className={`group hover:bg-[#272a31]/60 transition-colors cursor-pointer ${pacienteSeleccionado?.id === p.id ? 'bg-[#272a31]/80 border-l-2 border-[#00e0d3]' : 'border-l-2 border-transparent'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-[#32353c] text-[#00e0d3] flex items-center justify-center font-bold font-headline shadow-inner">
                            {p.nombre.charAt(0)}{p.apellido ? p.apellido.charAt(0) : ''}
                          </div>
                          <div>
                            <p className="font-bold text-[#e1e2eb]">{p.nombre} {p.apellido}</p>
                            <p className="text-xs text-[#bacac7] flex items-center gap-2">
                              {p.telefono && <><span className="material-symbols-outlined text-[12px]">call</span> {p.telefono}</>}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-mono text-sm text-[#bacac7]">{p.cedula || 'N/A'}</td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-[#e1e2eb]">{p.codigo || '--'}</p>
                        <p className="text-[10px] text-[#bacac7]">Internal Code</p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className="material-symbols-outlined text-[#bacac7] opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>chevron_right</span>
                      </td>
                    </tr>
                  ))}
                  ...
                  {resultados.ordenes.map(o => (
                    <tr key={`o-${o.id}`} className="group hover:bg-[#272a31]/40 transition-colors cursor-pointer border-l-2 border-transparent">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#bacac7]">{o.paciente}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-mono text-sm text-[#bacac7]">{o.numero_orden}</td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-[#e1e2eb]">{new Date(o.fecha).toLocaleDateString('es-DO')}</p>
                        <p className="text-[10px] text-[#bacac7]">Creation Date</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2 py-1 rounded bg-[#32353c] text-[#e1e2eb] text-[11px] font-medium border border-white/10">{o.estado}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className="material-symbols-outlined text-[#bacac7] opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>open_in_new</span>
                      </td>
                    </tr>
                  ))}
                  ...
                  {resultados.facturas.map(f => (
                    <tr key={`f-${f.id}`} className="group hover:bg-[#272a31]/40 transition-colors cursor-pointer border-l-2 border-transparent">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#bacac7]">{f.paciente}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-mono text-sm text-[#bacac7]">{f.ncf || 'Sin NCF'}</td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-[#e1e2eb]">{formatMoney(f.total)}</p>
                        <p className="text-[10px] text-[#bacac7]">Total Amount</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
          ...
          {pacienteSeleccionado && historial && (
            <aside className="w-96 bg-[#272a31] border-l border-white/5 flex flex-col relative overflow-hidden transition-all duration-300">
              <button onClick={() => { setPacienteSeleccionado(null); setHistorial(null); }} className="absolute top-4 right-4 text-[#bacac7] hover:text-[#ffb4ab] transition-colors z-10 w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/5">
              ...
              <div className="p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
                <div className="flex items-start gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-[#32353c] flex items-center justify-center border border-[#00e0d3]/20 text-[#00e0d3] font-headline text-2xl font-bold shadow-lg">
                  ...
                  <div className="mt-1">
                    ...
                    <p className="text-sm text-[#bacac7] font-mono mt-1">ID: {pacienteSeleccionado.cedula || 'N/A'}</p>
                  </div>
                </div>

                {/* Patient Metrics */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-[#32353c] p-3 rounded-xl border border-white/5">
                     <span className="text-[10px] uppercase font-bold tracking-wider text-[#bacac7] block mb-1">Blood Type</span>
                     <span className="text-[#e1e2eb] font-bold text-sm">{pacienteSeleccionado.tipo_sangre || 'Unknown'}</span>
                  </div>
                  <div className="bg-[#32353c] p-3 rounded-xl border border-white/5">
                     <span className="text-[10px] uppercase font-bold tracking-wider text-[#bacac7] block mb-1">Allergies</span>
                ...
                <div className="space-y-4">
                  <div className="bg-[#32353c] p-4 rounded-xl border border-white/5 relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-center mb-2 relative z-10">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-[#bacac7]">System Engagements</span>
                ...
                {/* Activity Log */}
                <div className="mt-2">
                  <h4 className="text-[10px] uppercase font-bold tracking-widest text-[#bacac7] mb-4">Recent Activity Log</h4>
                  <div className="space-y-6 relative before:content-[''] before:absolute before:left-2 before:top-2 before:bottom-0 before:w-px before:bg-white/10">
                    ...
                    {historial.ordenes.slice(0, 3).map((o, idx) => (
                      <div key={`ho-${o.id}`} className="relative pl-8">
                        <div className={`absolute left-0 top-1.5 w-4 h-4 rounded-full flex items-center justify-center border ${idx === 0 ? 'bg-[#4afdef]/20 border-[#4afdef]/40' : 'bg-[#32353c] border-white/20'}`}>
                        ...
                        <p className="text-[10px] text-[#bacac7] capitalize mt-0.5">{new Date(o.fecha_orden).toLocaleDateString('es-DO')} • {o.estado}</p>
                      </div>
                    ))}

                    {historial.facturas.slice(0, 2).map((f) => (
                      <div key={`hf-${f.id}`} className="relative pl-8">
                        <div className="absolute left-0 top-1.5 w-4 h-4 rounded-full bg-[#32353c] border border-white/20 flex items-center justify-center">
                        ...
                        <p className="text-[10px] text-[#bacac7] capitalize mt-0.5">{new Date(f.fecha_factura).toLocaleDateString('es-DO')} • {f.estado}</p>
                      </div>
                    ))}
                    
                    {historial.ordenes.length === 0 && historial.facturas.length === 0 && (
                       <p className="text-xs text-[#bacac7] pl-8 italic">No recent activities on record.</p>
                    )}
              ...
              <div className="mt-auto p-4 border-t border-white/5 bg-[#1d2026]">
                <button className="w-full py-3 bg-[#32353c] hover:bg-[#104f4a] transition-colors rounded-lg text-[#00e0d3] font-semibold text-sm flex items-center justify-center gap-2 border border-[#00e0d3]/20 shadow-[0_0_15px_rgba(0,224,211,0.05)]">
        ...
        {!pacienteSeleccionado && (
           <div className="flex justify-between items-center px-2 py-1">
             <span className="text-[10px] text-[#bacac7]/40 uppercase tracking-[0.2em] font-medium">Global Patient Repository Active</span>
             <span className="text-[10px] text-[#bacac7]/40 uppercase tracking-[0.2em] font-medium">Secured Node Connection</span>
           </div>
        )}
      </div>
    </div>
